  ...
  return (
    <main className="min-h-screen max-w-md mx-auto bg-black text-white px-4 py-5 flex flex-col justify-between font-sans pb-8">
      ...
      <section className="flex flex-col items-center w-full gap-2 mt-auto">
        {statusMessage && (
          <div className="text-[10px] font-mono text-emerald-400 bg-emerald-950/40 border border-emerald-800/60 px-3 py-1 rounded-md">{statusMessage}</div>
        )}
        ...
        <button
          ...
          className={`relative w-full py-3.5 rounded-xl font-mono text-xs font-bold tracking-widest uppercase flex items-center justify-center gap-2 ${
            action.disabled ? 'bg-zinc-900 text-zinc-600 border border-zinc-800' : 'bg-emerald-500 text-black'
          }`}
        >
